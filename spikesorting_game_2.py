...
tetrode_num = int(np.ceil((1 + chansel_pos) / 4.0))
...
n_channels = 10
pygame.init()
pygame.mixer.pre_init(44100, size=-16, channels=8, buffer=256)
pygame.mixer.init()
...
# Unit sound
unit_sound = pygame.mixer.Sound(unit_sound_path)
    ...
    if plot_units:
        ...
        for i_unit in range(n_unit_sel):
            ...
            for t_ap in times[i_unit]:
                ...
                pygame.draw.line(win, color_units, [line_ap_x, unit_offset_i - win_h/80],
                ...
                for j in range(4):
                    ...
                    pygame.draw.aalines(win, color_units, False, lines_ap, 5)
            ...
            if ap_to_play:
                for i_ap, (t_ap_sound, i_unit) in enumerate(ap_to_play):
                    if t_ap_sound < t_center_offset:
                        # unit_sound.play()
                        channels[i_chan].play(unit_sound, 0)
